...
class Input extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('backend/pegawai/model_input','',TRUE);
	...
	{
		$this->load->view('backend/pegawai/input_pemesan');
	}
	public function tamu($no_hp)
	{
		$data = array(
			'input' => $this->model_input->viewTamu($no_hp)
		);
		$this->load->view('backend/pegawai/input_tamu', $data);
	}
	public function inputPemesan()
	{
		...
		if(empty($nama_pemesan) OR empty($alamat_email) OR empty($no_hp) OR empty($jns_kamar) OR empty($jml_tamu_dewasa) OR empty($jml_tamu_anak) OR empty($jml_kamar) OR empty($tgl_cekin) OR empty($tgl_cekout)){	
			...
			redirect('backend/pegawai/Input');
			...
			redirect('backend/pegawai/Input/tamu/'.$no_hp);
		}
	}
